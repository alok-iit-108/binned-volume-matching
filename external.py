...
def sat2pol(prcoords, grcoords, re):
    ...
    r = (re+prcoords[..., 2])*denom/np.cos(np.deg2rad(theta)) #r_g in rob warren paper
...
def get_bb_ratio(pr_data, zp):
    ...
    bbwidth = pr_data['bbwidth'].copy()
    quality = pr_data['quality'].copy()
    ...
    return ratio, zbb, zbb_m, bbwidth_m 
